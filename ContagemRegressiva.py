...
import Armazenamento as armazenamento


def _converter_para_data(data_str: str):
    """Tenta converter uma string em objeto date. Suporta vários formatos."""
    if not data_str:
        return None
    ...
    for fmt in formatos:
        ...
        def _converter_para_data(data_str):
            if not data_str:
                return None
            formatos = ["%Y-%m-%d", "%d/%m/%Y", "%d-%m-%Y", "%Y/%m/%d"]
            for fmt in formatos:
        ...
        def dias_para_evento(data_evento):
            ...
            def _parse_table_txt(caminho):
                ...
                for linha in linhas[1:]:
                    ...
                    for i, h in enumerate(header):
                        reg_key = _normalizar_cabecalho(h)
                        reg[reg_key] = cols[i] if i < len(cols) else ''
                    registros.append(reg)
                return registros

            def _normalizar_cabecalho(token):
                t = token.lower()
                t = t.replace('ç', 'c')
                t = t.replace('ã', 'a').replace('á', 'a').replace('é', 'e').replace('í', 'i').replace('ó', 'o').replace('ú', 'u')
                t = t.replace(' ', '_')
                if 'nome' in t:
                    return 'nome'
                if 'esp' in t:
                    return 'especie'
                if 'ra' in t and 'raca' in t or 'ra' in t:
                    return 'raca'
                if 'idade' in t:
                    return 'idade'
                if 'sexo' in t:
                ...
                if 'estado' in t:
                    return 'estado_saude'
                if 'chegada' in t or 'data' in t:
                    if 'vac' in t or 'prev' in t or 'prevista' in t:
            ...
            def exibir_alertas(id_animal):
                ...
                eventos = [c for c in cuidados if c.get('animal_id') == id_animal or c.get('animal_id') == (animal.get('id') if animal else None)]
                if not eventos:
